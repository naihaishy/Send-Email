...
function wp_mail_smtp_options_page() {
	...
	if (isset($_POST['wpms_action']) && $_POST['wpms_action'] == '发送测试邮件' && isset($_POST['subject']) && isset($_POST['message']) && isset($_POST['to'])) {
...
<tr valign="top">
<th scope="row"><label for="message">内容</label></th>
<td><input name="message" type="text" id="message" value="" size="40" class="code" />
<span class="description">请输入发信内容。</span></td>
</tr>
</table>
...
function wp_mail_smtp_menus() {
	...
	if (function_exists('add_submenu_page')) {
		add_options_page(__('邮件设置', 'wp_mail_smtp'),__('邮件设置', 'wp_mail_smtp'),'manage_options','send_mail_settings', 'wp_mail_smtp_options_page');
	}
